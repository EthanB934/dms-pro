...
import {
  getDoorsByCoolerId,
  updateDoor,
} from "../../services/DoorServices/doors";
...
export const EditCooler = ({ token }) => {
  ...
  useEffect(() => {
    if ("id" in cooler) handleGetCoolerDoors();
  }, [token, cooler]);
      ...
      <section className="doors">
        ...
        <ul className="doors-list">
          {doors.map((door, index) => {
            return (
              <div className="pop-ups" key={door.id}>
                <li
                  className="door"
                  key={door.id}
                ...
                {door.id === doorId && !showModal ? (
                  ...
                  >
                    <fieldset className="door-fields">
                      ...
                      <DoorType
                        setChoice={setTypeId}
                        coolerTypes={cooler.types}
                        token={token}
                      />
                      <br />
                      <button id="door-update">Update Cooler Door</button>
                    </fieldset>
                  </form>
                ) : (
                  " "
                )}
                {showModal && door.id === doorId ? (
                  <>
                    <DeleteModal
                      door={doorToDelete}
                      refetcher={handleGetCoolerDoors}
